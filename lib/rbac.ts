...
export const navigationItems = [
  ...
  { name: 'Campaigns', href: '/campaigns', icon: 'Target', resource: 'campaigns', action: 'read' },
  { name: 'Content Calendar', href: '/content', icon: 'Calendar', resource: 'content', action: 'read' },
  { name: 'Leads', href: '/leads', icon: 'MessageSquare', resource: 'leads', action: 'read' },
  { name: 'Tasks', href: '/tasks', icon: 'Zap', resource: 'tasks', action: 'read' },
  { name: 'Analytics', href: '/analytics', icon: 'BarChart3', resource: 'analytics', action: 'read' },
  { name: 'Invoices', href: '/invoices', icon: 'FileText', resource: 'invoices', action: 'read' },
];
 ...
 */
export function getRoleBasedStats(userRole: string, userName: string, allData: any) {
  const { clients, campaigns, content, leads, tasks, invoices } = allData;

  const filteredClients = filterDataByRole(clients, userRole, userName, 'clients');
  const filteredCampaigns = filterDataByRole(campaigns, userRole, userName, 'campaigns');
  const filteredContent = filterDataByRole(content, userRole, userName, 'content');
  ...
  const filteredInvoices = filterDataByRole(invoices, userRole, userName, 'invoices');

  return {
    totalClients: filteredClients.length,
    activeCampaigns: filteredCampaigns.filter((c: any) => c.status === 'Active').length,
    ...
    pendingTasks: filteredTasks.filter((t: any) => ['Todo', 'In Progress'].includes(t.status)).length,
    completedTasks: filteredTasks.filter((t: any) => t.status === 'Done').length,
    monthlyRevenue: hasPermission(userRole, 'invoices', 'read') 
      ? filteredInvoices.filter((i: any) => i.status === 'Paid').reduce((sum: number, i: any) => sum + i.amount, 0)
      : 0,
  };
}
